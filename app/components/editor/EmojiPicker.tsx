...
const EmojiPicker_: React.FC<PickerProps> = ({
  ...
  for (const emoji of validCustomEmojis) {
    ...
    customEmojisData[id] = {
      id,
      name: emoji.name,
      keywords: ["discord", emoji.name, emoji.animated ? "animated" : "static"],
  ...
  // Build guild-based categories from cache.customEmojiGuilds
  const guildCategories = useMemo(() => {
    const guilds = cache?.customEmojiGuilds ?? [];
    if (guilds.length === 0 && validCustomEmojis.length > 0) {
      // Fallback: single custom category if no guild data
      return [{
        id: "custom",
        label: "CUSTOM",
    ...
    return guilds
      ...
      .map((guild) => ({
        id: `guild_${guild.guild_id ?? "global"}`,
        label: guild.name ?? (guild.guild_id ? guild.guild_id : "App Emojis"),
        icon_url: guild.icon_url,
        emojis: guild.emojis.map((e) => `discord_${e.id}`),
      }));
  }, [cache?.customEmojiGuilds, validCustomEmojis]);

  const categories: (Category & { icon_url?: string | null; label?: string })[] = [
    ...guildCategories,
    ...data.categories,
  ];
        ...
        <>
          ...
          <div className="flex grow h-full overflow-hidden">
            <div className="w-10 shrink-0 bg-gray-400 dark:bg-gray-900 overflow-y-auto h-full ltr:rounded-bl-lg rtl:rounded-br-lg scrollbar-none space-y-1 p-1 py-2 flex flex-col">
              {categories
                .filter(
                  (c) => c.id.startsWith("guild_") || (c.id === "custom" && cache) || c.emojis.length > 0,
                )
                .map((category) => {
                  const isGuild = category.id.startsWith("guild_") || category.id === "custom";
                  const iconUrl = (category as any).icon_url;
                  return (
                    <button
                      type="button"
                      key={`emoji-category-${id}-${category.id}-icon`}
                      className="block mx-auto rounded hover:bg-black/10 dark:hover:bg-white/10 p-0.5"
                      onClick={() => {
                        const sectionHeader = document.getElementById(`${id}-${category.id}`);
                        if (sectionHeader) {
                          sectionHeader.scrollIntoView();
                        }
                      }}
                    >
                      {isGuild && iconUrl ? (
                        ...
                        categoryToIcon[category.id]?.()
                          ?? <CoolIcon icon="Paperclip_Attechment_Horizontal" />
                      )}
                    </button>
                  );
                })}
              ...
              <div className="grow px-1.5 pb-1">
                {query
                  ? Object.values(data.emojis)
                      .filter(
                        (e) =>
                          ...
                          e.name.toLowerCase().includes(query.replace(/ /g, "_")) ||
                          e.keywords
                  ...
                  : categories
                      .filter(
                        (c) =>
                          c.id.startsWith("guild_") || (c.id === "custom" && cache) || c.emojis.length > 0,
                      )
                      .map((category) => (
                        <div
                          ...
                          >
                            {(category as any).icon_url ? (
                              <img
                                ...
                                alt=""
                                className="w-4 h-4 rounded-full my-auto ltr:mr-1.5 rtl:ml-1.5"
                              />
                            ) : (
                              categoryToIcon[category.id]?.({
                            ...
                            <p className="my-auto">
                              {(category as any).label ?? category.id}
                            </p>
                          </div>
                          <div className="flex gap-px flex-wrap">
                            {(category.id.startsWith("guild_") || category.id === "custom") && cache && (
                              <button
                                type="button"
