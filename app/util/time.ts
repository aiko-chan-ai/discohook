...
export const cronDaysOfWeek = [...Array(7).keys()];
export const cronDaysOfMonth = [...Array(31).keys()].map(add);
export const cronMonths = [...Array(12).keys()].map(add);
export const cronHours = [...Array(24).keys()];
export const cronMinutes = [...Array(60).keys()];
...
export const cronToEnglish = ({
  ...
  if (minute.length === cronMinutes.length) {
    ...
    const times = hour
      .map((h: number) => minute.map((m: number) => clock(h, m)))
